...
public class Main {
	...
	class QueueOfTiles {
		//acts as linear queue storing the next lettersto be added to the players' tile lists
		...
		char remove() // returns the item at the front of the queue, moves the positions of the other items closer to the front
		{
			if (isEmpty()) {
				return '\n';
			}
		...
		void add() // adds a random letter to the rear position of the queue, unless the queue is full
		{
		...
		void show() //prints each item in the queue, assumingthe queue isn't empty
		{
			if (rear != -1) 
			{
				Console.println();
	...
	Map createTileDictionary() //creates a map, assigns a points value to each letter
	{
		...
		for (int count = 0; count < 26; count++) 
		{
			...
			case 18:
			case 19:
				tileDictionary.put((char)(65 + count), 1);	//the corresponding letters are worth 1 point each
				break;
			case 1:
			case 2:
			case 3:
	...
	String[] loadAllowedWords()
	{
		String[] allowedWords = {};
		try {
			Path filePath = new File("mostcommonwords.txt").toPath();
	...
	boolean checkWordIsValid(String word, String[] allowedWords)
	{
		...
		*/
		boolean validWord = binarySearch(word, allowedWords, 0, allowedWords.length - 1);
		return validWord;
	}
	
	boolean binarySearch(String word, String[] allowedWords, int l, int r){
		int m = (l + r) / 2;
		if(word.equals(allowedWords[m])){
			return true;
		}
		else if(l == r - 1){
			return false;
		}
		else if(word.compareTo(allowedWords[m]) < 0){	//if the word is alphabetically before the midpoint
			return binarySearch(word, allowedWords, l, m);	//check the first half
		}
		else if(word.compareTo(allowedWords[m]) > 0){	//if the word is alphabetically after the midpoint
			return binarySearch(word, allowedWords, m, r);	//check the second half
		}
		return false;
	}
